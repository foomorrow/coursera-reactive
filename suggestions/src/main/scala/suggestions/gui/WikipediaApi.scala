...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs.map(s => s.replaceAll("""\s""","_"))
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(s => Success(s)).onErrorReturn{
      case e => Failure(e)
    }
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = obs.timeout(Duration(totalSec,SECONDS)).first
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      obs.map(requestMethod)
        .concatMap(s => s.recovered)
  }
